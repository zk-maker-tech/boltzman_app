...
from custom_legend_handler import HandlerInterspersedText
...
# --- 配置信息 ---
file_path_curves = 'fit.xlsx'  # 曲线数据 (预计)
file_path_markers = 'coal_data.xlsx'  # 标记点数据 (实际) <--- MAKE SURE THIS LINE IS PRESENT
q_columns = [f'Q{i:03}' for i in range(11, 21)]  # Q列的列名
x_coordinates_q = np.array([  # Q系列对应的X坐标
    -39.105, -20.13, 4.093, 34.095, 45.119, 73.094,
    97.966, 122.818, 147.73, 168.246
])
prediction_time_key_options = ['Prediction_time', 'Prediction_Time']  # Excel中时间列可能的列名
plot_filename_q = 'q_series_plot_point_based_handler_v2.png'  # 更新文件名


def get_key_column(df, options):
    """
    从提供的选项中获取DataFrame中存在的列名。
    """
...
try:
    ...
    try:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"无法从 '{file_path_curves}' (Excel 或常见 CSV 变体) 加载曲线数据. CSV 错误: {e_curves_csv}")
    print(f"曲线 (预计) 数据已加载. 列名: {df_curves.columns.tolist()}")

    # 2. 加载标记点数据 ("实际")
    try:
        df_markers = pd.read_excel(file_path_markers, sheet_name=0)  # 使用 file_path_markers
        ...
        try:
            df_markers = pd.read_csv('coal_data.xlsx - up.csv')  # CSV文件名应与file_path_markers对应（如果也是CSV）
            print(f"成功从 CSV 加载标记数据: coal_data.xlsx - up.csv")
        except Exception as e_markers_csv:
            raise FileNotFoundError(
                f"无法从 '{file_path_markers}' (Sheet 0, Excel 或 'coal_data.xlsx - up.csv') 加载标记数据. CSV 错误: {e_markers_csv}")
    ...
    for current_pred_time_val_str in unique_prediction_times_str:
        ...
        if marker_legend_proxy and has_actual_data_for_row:
            ...
            legend_labels_list.append("")  # Label is empty for custom handler
    ...
    if num_legend_entries > 0:
        ...
        legend_font_size = 10  # 使用明确的磅值，例如 10pt
        ...
        if num_legend_entries > 7:
            ax.legend(legend_handles_list, legend_labels_list, title=legend_title,
                      ...
                      numpoints=1,
                      markerscale=0.8)  # 调整 markerscale
    ...
    # 请您再次检查脚本中是否有其他地方可能绘制了图片左下角的 “图例：实际 (时间t)” 额外文字的代码。
    # 例如，查找 ax.text(), fig.text(), plt.text() 或其他 ax.legend() 调用。
